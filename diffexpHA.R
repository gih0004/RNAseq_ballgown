...
sample<-c("HA_10D_2_ACCTTGGC-GGCCTCAT_S39_L002.sunflower.sam.sort", "HA_10D_3_ATATCTCG-ATCTTAGT_S40_L002.sunflower.sam.sort", "HA_20D_2_GCGCTCTA-GCTCCGAC_S41_L002.sunflower.sam.sort", "HA_20D_3_AACAGGTT-ATACCAAG_S42_L002.sunflower.sam.sort", "HA_35D_2_TGGTGGCA-TCCTGTAA_S45_L002.sunflower.sam.sort","HA_35D_3_AGGCAGAG-AGAATGCC_S46_L002.sunflower.sam.sort" )
dpf<-c(rep("10D", 2), rep("20D", 2), rep("35D", 2))
...
pheno_df<-data.frame("sample"=sample,"dpf"=dpf)
...
bg1= subset(bg,"dpf != '35D'", genomesubset= FALSE)
bg2=subset(bg, "dpf!='10D'", genomesubset=FALSE)
bg3=subset(bg,"dpf!='20D'", genomesubset=FALSE)
save(bg3, file='bg3.rda1')
...
transcript_fpkm = texpr(bg3, 'FPKM') # fragments per kilobase of transcript per million mapped reads
transcript_cov = texpr(bg3, 'cov') # average per base read coverage
whole_tx_table = texpr(bg3, 'all') # all transcript level expression data with extra metadata
exon_mcov = eexpr(bg3, 'mcov') # multi map corrected average per base read coverage
junction_rcount = iexpr(bg3) # intron expression
whole_intron_table = iexpr(bg3, 'all') #intron level expression data with extra metadata
gene_expression_bg3 = gexpr(bg3) # gene expression in FPKM

exon_transcript_table_bg3 = indexes(bg3)$e2t # exons to transcripts table
transcript_gene_table_bg3 = indexes(bg3)$t2g # transcripts to genes table


write.csv(transcript_gene_table_bg3, "transcript_gene_table_bg3.csv")

head(transcript_gene_table_bg3)

head(gene_expression_bg3)
...
save(bg3_filt, file='bg3_filt.rda')
...
results_transcripts_bg3 = stattest(bg3_filt, feature="transcript", meas = "FPKM" , covariate = "dpf", getFC="TRUE" ) #covariate is what your comparing between samples (could be genotype or time points )
...
results_genes_bg3 = stattest(bg3_filt, feature="gene" , covariate = "dpf" , meas = "FPKM" , getFC="TRUE")
results_genes_order_bg3<- results_genes_bg3[ order (-results_genes_bg3$fc),]
write.csv(results_genes_order_bg3, "results_gene_order_bg3.csv")

#Add gene names and gene IDs to the results_transcripts data frame:
results_transcripts_bg3 = data.frame(geneNames=ballgown::geneNames(bg3_filt), geneIDs=ballgown::geneIDs(bg3_filt), results_transcripts_bg3)


#Sort the results from the smallest P value to the largest:
results_transcripts_bg3 = arrange(results_transcripts_bg3,pval)
results_genes_bg3 = arrange(results_genes_bg3,pval)

#The log fold change (logFC) can be computed.
results_transcripts_bg3$logFC <- log2(results_transcripts_bg3$fc)
results_genes_bg3$logFC <- log2(results_genes_bg3$fc)


#Write the results to a csv file that can be shared and distributed: Identify transcripts and genes with a q value <0.05:
write.csv(results_transcripts_bg3, "transcript_results_bg3.csv", row.names=FALSE)
write.csv(results_genes_bg3, "gene_results_bg3.csv", row.names=FALSE)
...
results_transcripts_bg3_q0.05 <- subset(results_transcripts_bg3,results_transcripts_bg3$qval<0.05)
results_genes_bg3_q0.05 <- subset(results_genes_bg3,results_genes_bg3$qval<0.05)

#Write the results to a csv file that can be shared and distributed: Identify transcripts and genes with a q value <0.05:
write.csv(results_transcripts_bg3_q0.05, "transcript_results__bg3_q0.05.csv", row.names=FALSE)
write.csv(results_genes_bg3_q0.05, "gene_results_bg3_q0.05.csv", row.names=FALSE)
...
volcano_plot(results_genes_bg3)
...
logfc.threshold <- 5
ggplot() + 
  geom_point(data = subset(results_genes_bg3, qval>.05 & abs(logFC)<logfc.threshold), aes(x = logFC, y = -log10(qval)), fill = "grey", shape = 21, alpha = 0.2) + 
  geom_point(data = subset(results_genes_bg3, qval<0.05), aes(x = logFC, y = -log10(qval)), fill = cbbPalette[[7]], shape = 21) + 
  geom_point(data = subset(results_genes_bg3, abs(logFC)>logfc.threshold), aes(x = logFC, y = -log10(qval)), fill = cbbPalette[[5]], shape = 21) + 
  geom_point(data = subset(results_genes_bg3, qval<.05 & abs(logFC)>logfc.threshold), aes(x = logFC, y = -log10(qval)), fill = cbbPalette[[3]], shape = 21) +
  theme_classic() +
  geom_text_repel(data = subset(results_genes_bg3, -log10(qval)>10 & abs(logFC)>2), aes(x = logFC, y = -log10(qval), label = id), size = 2)
...
#Plot transcripts
sampleNames(bg3_filt)

plotTranscripts(ballgown::geneIDs(bg3_filt)[ballgown::geneIDs(bg3_filt) == "MSTRG.13012"], bg3_filt, main=c('Gene CLV3'), sample=c('HA_10D_2_ACCTTGGC-GGCCTCAT_S39_L002.sunflower.sam.sort', 'HA_10D_3_ATATCTCG-ATCTTAGT_S40_L002.sunflower.sam.sort', 'HA_35D_2_TGGTGGCA-TCCTGTAA_S45_L002.sunflower.sam.sort', 'HA_35D_3_AGGCAGAG-AGAATGCC_S46_L002.sunflower.sam.sort'), labelTranscripts = TRUE)


plotTranscripts(ballgown::geneIDs(bg3_filt)[ballgown::geneIDs(bg3_filt) == "MSTRG.13012"], bg3_filt, main=c('CLV3 10D_2'), sample=c('HA_10D_2_ACCTTGGC-GGCCTCAT_S39_L002.sunflower.sam.sort'), legend = TRUE, labelTranscripts = TRUE)
plotTranscripts(ballgown::geneIDs(bg3_filt)[ballgown::geneIDs(bg3_filt) == "MSTRG.13012"], bg3_filt, main=c('CLV3 35D_2'), sample=c('HA_35D_2_TGGTGGCA-TCCTGTAA_S45_L002.sunflower.sam.sort'), legend = TRUE, labelTranscripts = TRUE)

plotTranscripts(ballgown::geneIDs(bg3_filt)[ballgown::geneIDs(bg3_filt) == "MSTRG.13012"], bg3_filt, main=c('CLV3 10D_3'), sample=c('HA_10D_3_ATATCTCG-ATCTTAGT_S40_L002.sunflower.sam.sort'), legend = TRUE, labelTranscripts = TRUE)
plotTranscripts(ballgown::geneIDs(bg3_filt)[ballgown::geneIDs(bg3_filt) == "MSTRG.13012"], bg3_filt, main=c('CLV3 35D_3'), sample=c('HA_35D_3_AGGCAGAG-AGAATGCC_S46_L002.sunflower.sam.sort'), legend = TRUE, labelTranscripts = TRUE)


# different isoforms of differntially expressed genes
plotMeans('MSTRG.13012', bg3_filt, groupvar ="dpf", legend=TRUE) 
...
input_file <- ("C:/Users/guill/Rstudio/OmicsInAG/ballgown/transcript_results_bg3.csv")

#read the file into a dataframe
diff_df_bg3 <- read.delim(file=input_file,header=TRUE,sep=',')

#check some attributes of the data
colnames(diff_df_bg3)

dim(diff_df_bg3)

# keep only the fields needed for the plot
# FDR = false discovery rate = adjusted p value = significance 
diff_df_bg3 <- diff_df_bg3[c("geneIDs", "logFC", "qval")]

# preview the dataset; data required for the plot
head(diff_df_bg3)

# add a grouping column; default value is "not significant"
diff_df_bg3["group"] <- "NotSignificant"
...
diff_df_bg3[which(diff_df_bg3['qval'] < 0.05 & abs(diff_df_bg3['logFC']) < 1.5 ),"group"] <- "Significant"

# change the grouping for the entries a large enough Fold change but not a low enough p value
diff_df_bg3[which(diff_df_bg3['qval'] > 0.05 & abs(diff_df_bg3['logFC']) > 1.5 ),"group"] <- "FoldChange"

# change the grouping for the entries with both significance and large enough fold change
diff_df_bg3[which(diff_df_bg3['qval'] < 0.05 & abs(diff_df_bg3['logFC']) > 1.5 ),"group"] <- "Significant&FoldChange"


# Find and label the top peaks..
top_peaks_bg3 <- diff_df_bg3[with(diff_df_bg3, order(logFC, qval)),][1:8,]
top_peaks_bg3 <- rbind(top_peaks_bg3, diff_df_bg3[with(diff_df_bg3, order(-logFC, qval)),][1:8,])
...
a <- list()
for (i in seq_len(nrow(top_peaks_bg3))) {
  m <- top_peaks_bg3[i, ]
  a[[i]] <- list(
    x = m[["logFC"]],
    y = -log10(m[["qval"]]),
...
diff_df_bg3
save(diff_df_bg3, file='diff_df10to35')
# make the Plot.ly plot
p <- plot_ly(data = diff_df_bg3, x = ~logFC, y = ~-log10(qval), text = geneIDs, color = ~group) %>% add_trace(type = "scatter", mode = "markers")%>%
